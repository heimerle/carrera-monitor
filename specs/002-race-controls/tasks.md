...
- [X] T028 [US2] Add `RaceService.is_safety_car_active(race_id: int) -> bool` reading `self._safety_car_active`; on cache miss, reconcile lazily from the latest `safety_car_started` / `safety_car_ended` event row via `RaceRepository.latest_event_type(...)` so the flag survives a process restart (FR-211 / spec §SafetyCarFlag polish).
