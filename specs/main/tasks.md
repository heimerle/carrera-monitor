...
- [X] T029 [US2] Update `src/main.py` to choose `LiveCarreraAdapter` (via `CarreraClientRunner`) when `--mock` is absent; surface `--mac` to override `bluetooth.mac_address`; honor exit code 2 on `--mock` + `--mac` mutual exclusion per CLI contract.
