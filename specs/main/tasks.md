...
**Tests**: INCLUDED. The feature spec (FR-034) explicitly requires pytest coverage for event model, storage writer, mock telemetry generator, and state manager. Tests for the adapter contract are added to support US2.
...
- `MockCarreraAdapter` (T016, T021) and `JsonlEventWriter` (T017) and `StateManager` (T018) before `main.py` orchestration (T023).
...
1. **MVP first**: complete Phases 1, 2, 3 → ship US1 (`python -m src.main --mock`). This satisfies every "Acceptance Criteria" bullet in the spec except live-hardware reconnect.
