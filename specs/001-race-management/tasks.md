...
- [ ] T036 [US3] Implement the full UI in [src/pages/race_reports.py](src/pages/race_reports.py) (replacing the US1 stub): race selector (only finished/cancelled races), standings table, driver-stats table, race duration, "Save Report" button (free-form report-type input), saved-reports list, **two** `st.download_button`s — "Download Summary CSV" wired to `export_summary_csv(race_id)` and "Download Laps CSV" wired to `export_laps_csv(race_id)` (FR-126); NO direct SQLAlchemy imports (FR-130)
