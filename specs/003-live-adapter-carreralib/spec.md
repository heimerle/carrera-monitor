...
- **FR-004** `[X]`: System MUST implement a BLE idle watchdog that, when no `Status` or `Timer` event has been observed for a configurable interval, forces a reconnect through the same path used for hard drops.
